...
int main() {
        ...
        if (option == 1) {
            ...
            if (found_count == 0) {
                ...
                for (int i = 0; i < found_count && i < 5; i++) {
                    // Usamos extract_filename para mostrar solo el nombre (sin la ruta completa)
    ...
    if (rev_index == NULL) {
        ...
        if (num_results == 0) {
            printf("No se encontraron documentos con '%s' usando búsqueda eficiente.\n", query_text);
        } else {
            ...
            for (int i = 0; i < num_results && i < 5; i++) {
                ...
                if (d) {
                    float relevancia = score_textual + alpha * graphGetIndegree(global_graph, doc_ids[i]);
                    printf("[%d] %s - Relevancia: %.1f\n", i, extract_filename(d->filename), relevancia);
                }
            }
