...
void index_documents(HashMap *map, DocumentGraph *graph, Document *docs) {
    ...
    for (Document *d = docs; d != NULL; d = d->next, doc_id++) {
        ...
        char word[64];
        char indexed_words[500][64];  // Registro de palabras ya indexadas en este documento
        int indexed_count = 0;

        while (fscanf(file, "%63s", word) == 1) {
            // Detectar enlaces en el texto (formato: "(ID)")
            if (word[0] == '(') {
                int linked_doc = atoi(word + 1);  // Obtener el ID del documento al que apunta el enlace
                if (linked_doc >= 0) {
                    add_link(graph, doc_id, linked_doc);  // Agregar relación en el grafo dirigido
                }
            }
            ...
            while (entry) {
                if (strcmp(entry->word, word) == 0) {
                    entry->documentIds = realloc(entry->documentIds, (entry->count + 1) * sizeof(int));

                    // Asegurar que los documentos se agreguen en orden numérico ascendente
                    int pos = entry->count;
