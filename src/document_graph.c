...
// (El parámetro 'fromDoc' nos permite registrar de dónde viene el enlace, aunque para el
// cálculo de relevancia solo necesitamos incrementar el indegree del documento destino)
void add_link(DocumentGraph *graph, int fromDoc, int toDoc) {
    if (toDoc < 0 || toDoc >= MAX_DOCUMENTS || fromDoc < 0 || fromDoc >= MAX_DOCUMENTS)
    ...
    // Si aún no existe un nodo para 'fromDoc', lo creamos (para rastrear enlaces salientes)
...
float graphGetIndegree(DocumentGraph *graph, int documentId) {
    if (!graph || documentId < 0 || documentId >= MAX_DOCUMENTS || graph->nodes[documentId] == NULL)
        return 0.0;
